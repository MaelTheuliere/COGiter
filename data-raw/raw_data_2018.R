...
epci_type<-read_excel("data-raw/source/2018/COG/Intercommunalité - Métropole au 01-01-2018.xls",sheet=1,skip=5) %>%
...
epci<-read_excel("data-raw/source/2018/COG/Intercommunalité - Métropole au 01-01-2018.xls",sheet=2,skip=5) %>%
...
departements<-read.delim("data-raw/source/2018/COG/depts2018.txt",encoding = "latin1") %>%
  mutate(REGION=REGION %>% as.character(.) %>% str_pad(.,2,"left",0) %>% as.factor) %>%
  rename(REG=REGION,
         NOM_DEP=NCCENR) %>%
  as_tibble()

regions<-read.delim("data-raw/source/2018/COG/reg2018.txt",encoding = "latin1")  %>%
  mutate(REGION=REGION %>% as.character(.) %>% str_pad(.,2,"left",0) %>% as.factor)%>%
  rename(REG=REGION,
         NOM_REG=NCCENR) %>%
  as_tibble()

communes<-read.delim("data-raw/source/2018/COG/comsimp2018.txt",encoding="latin1") %>%
...
table_passage_com_epci<-read_excel("data-raw/source/2018/COG/Intercommunalité - Métropole au 01-01-2018.xls",sheet=2,skip=5) %>%
...
#Zonages abc
zonage_abc_r52<-read_excel("data-raw/source/2018/zonageabc_pdl_communes2018.xls") %>%
  mutate(zonage_abc=str_c("Zone ",zonage_abc)) %>%
  mutate_all(funs(as.factor(.)))

zonage_pinel_r52<-read_excel("data-raw/source/2018/zonagepinel_pdl_communes2018.xls") %>%
